...
Use before final verification for non-trivial tickets unless `spec_refs.mode:
no_spec` is justified in the ticket.
...
- [ ] `execution_result.spec_alignment.verdict` is `pass`, `pass_with_risk`, or
      `fail`.
- [ ] `execution_result.spec_alignment.checked_specs`, `drift_findings`, and
      `required_followups` are complete.
