...
- Ticket 12 release commit is `f42f359212b4ba3a364c684fddab019cfcf7cd85` on `main`, followed by scoped proof-record commits.
- After each final delivery push, the manager verifies local `HEAD` equals `origin/main`; the exact current SHA is the Git command output, not a hardcoded tracked-file constant.
